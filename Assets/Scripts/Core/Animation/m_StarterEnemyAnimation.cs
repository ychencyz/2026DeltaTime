...
public class m_StarterEnemyAnimation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        SetSpeed();
        SetVelocities();
        JumpAndGravity(); //砍到隻剩掉落計算
    }

    public void SetVelocities()
    {
        _animator.SetFloat("Velocity X", agent.velocity.x);
        _animator.SetFloat("Velocity Z", agent.velocity.z);
    }
    ...
    private void SetSpeed()
    {
        ...
        //TODO:之後補上 in combat speed
        bool inCombat = false;
        float targetSpeed = inCombat ? SprintSpeed : MoveSpeed;

        // a simplistic acceleration and deceleration designed to be easy to remove, replace, or iterate upon

        // note: Vector2's == operator uses approximation so is not floating point error prone, and is cheaper than magnitude
        // if there is no input, set the target speed to 0
        if (agent.velocity.magnitude == 0f) targetSpeed = 0.0f;
